...
open class DefaultTypedProperties protected constructor (
    ...
    override fun merge(source: TypedProperties) {
        merge(source, source.keys)
    }

    override fun merge(source: TypedProperties, vararg keys: Marker) {
        merge(source, keys.toList())
    }

    override fun merge(source: TypedProperties, keys: Collection<Marker>) {
        for (key in keys) {
            map[key] = source[key]
        }
    }
    ...
    override fun putAll(from: TypedProperties) = map.putAll(from)
